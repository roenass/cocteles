...
#let cocktail(label: none, threshold: 7em, cocktail, recipe) = stick(threshold: threshold, [#index[#cocktail] #heading(level:2)[#cocktail]#label],[#v(-3pt)#recipe])
#let moviecocktail(label: none, threshold: 7em, cocktail, movie, recipe) = stick(threshold: threshold, [#index[#cocktail] #heading(level:2)[#cocktail #text(cocktailcolor)[☛ #emph[#movie]]]#label],[#v(-3pt)#recipe])
...
#set page(margin: (top: 1.8cm, bottom: 1.8cm))
...
#cocktail(label: <queenelizabeth>, [Queen Elizabeth],[
- 2 oz #ingredient([Gin])
- ½ oz #ingredient([Orange Liqueur])
- ½ oz #ingredient([Lemon Juice])
...
#cocktail(label: <glitterati>, [Glitterati],[
- 2 oz #ingredient([Vodka])
...
#cocktail([Merry Widow],[See @merrywidow.])

#cocktail([Journalist],[See @journalist.])

#cocktail([Hammett Martini],[See @hammettmartini.])

#cocktail([Martini (a la Luis Buñuel)],[See @martinialaluisbunuel.])

#cocktail([Balanced Account],[See @balancedaccount.])

#cocktail([The Better Wife],[See @thebetterwife.])
])
...
#cocktail([Hemingway Champagne],[Another name for  *Death in the Afternoon* (@deathintheafternoon).])#indexIn([Bubbles])

#moviecocktail([Death Spasm],[Cocktail],[The *Death in the Afternoon* (@deathintheafternoon), adapted for poetic purposes.])#indexIn([Bubbles])

#cocktail(threshold: 8em, [Pornstar Martini],[
...
#cocktail([Cuba Libre],[*Dirty Pirate Popsicles* (@dirtypirateposicles) by another name.])

#cocktail([Baco],[*Dirty Pirate Popsicles* (@dirtypirateposicles) once more.])
...
#cocktail(threshold: 8em, label: <moscowmule>, [Moscow Mule],[
- 1½ oz #ingredient([Vodka])
...
#cocktail(threshold: 9em, [Gin Rickey],[
- 2 oz #ingredient([Gin])
- ½ oz #ingredient([Lime Juice])
- #ingredient([Sparkling Water])
...
#cocktail(threshold: 9em, [Bloodless Mary],[
- 2 oz #ingredient([Vodka])
- ½ oz #ingredient([Lemon Juice])
- 1 dash #ingredient([Tabasco])
- 1 oz #ingredient([Sparkling Wine]) #indexIn([Bubbles])
...
#cocktail([Flat on my Flute],[See @flatonmyflute.])

#cocktail([Champagne Cocktail],[See @champagnecocktail.])

#cocktail([Amber Mimosa],[See @ambermimosa.])

#cocktail([Apple Crisp],[See @applecrisp.])

#cocktail([Chinese Show],[See @chineseshow.])

#cocktail([Sunglow],[See @sunglow.])

#cocktail([Vine],[See @vine.])

#cocktail([Bamboozle],[See @bamboozle.])

#cocktail([Iced Tea Dance],[See @icedteadance.])

#cocktail([Cool Dude],[See @cooldude.])

#cocktail([Berry Well],[See @berrywell.])

#cocktail([The Dead Reckoning],[See @deadreckoning.])

#cocktail([Horse’s Neck],[See @horsesneck.])

#cocktail([El Diablo],[See @eldiablo.])

//#colbreak()

#cocktail([The Grable],[See @grable.])

#cocktail([Kitty Collins],[See @kittycollins.])

#cocktail([Reckless Moment],[See @recklessmoment.])

#cocktail([Gin Sling],[See @ginsling.])

#cocktail([Stranger’s Kiss],[See @strangerskiss.])

#cocktail([Tom Powers],[See @tompowers.])

#cocktail([Last Flight],[See @lastflight.])

#cocktail([Javelin],[See @javelin.])

#cocktail([Red Dust],[See @reddust.])

#cocktail([Naked Moon],[See @nakedmoon.])

#cocktail([Diamond Ring],[See @diamondring.])

#cocktail([Kansas Romanov],[See @kansasromanov.])

#cocktail([Midnight Mary],[See @midnightmary.])#index([Red Snapper])

#cocktail([Red Snapper],[See @midnightmary.])#index([Midnight Mary])

#cocktail([Jardin d’Orient],[See @jardindorient.])
])
...
#cocktail([Kopstoot],[A *Boilermaker* (@boilermaker).])
...
Stir liquids with ice. Strain into ice-filled tumbler.])
...
#cocktail([Stolly & Bolly],[The *Ab Fab* (@abfab) by another name.])
...
#moviecocktail([Billericay Dickie],[Sex & Drugs & Rock & Roll],[A.k.a. *Pomegranate Negroni* (@pomegranatenegroni). No doubt the late Ian Dury would have approved.])
...
#cocktail(threshold: 9em, [Vanilla Ice],[
...
#cocktail(threshold: 10em, [Boulevardier],[
- 1¼ oz #ingredient([Bourbon]) #indexIn([Whiskey])
- 1 oz #ingredient([Campari])
- 1 oz #ingredient([Sweet Vermouth])
- #ingredient([Orange]) Peel for garnish
...
Garnish by floating a few leaves of mint.])
...
#heading(level: 1, outlined: false, numbering: none)[Lost Recipes]

#cocktail([Currents of Jordan],none)

#cocktail([First Kiss],none)

// Kus van Keetje Tippel
#cocktail([Nell Kimball’s Kiss],[Known in dutch as De Kus van Keetje Tippel#index[De Kus van Keetje Tippel]])

#cocktail([Inez Armand],none)

#cocktail(threshold: 1em, [The Kiss Forced],none)
...
#moviecocktail([Merry Widow], [Shadow of a Doubt],[See @merrywidow.])
...
#moviecocktail([Champagne Cocktail], [Sunset Blvd.],[See @champagnecocktail.])
...
#moviecocktail([Legionnaire], [Morocco],[See @legionnaire.])
...
#moviecocktail([First Ward], [Scarface],[See @firstward.])

#moviecocktail([Lil Red], [Red-Headed Woman],[See @lilred.])
...
#moviecocktail([Flat on my Flute], [Love Me Tonight],[See @flatonmyflute.])
...
- *#ingredient([Champagne])*: Taittinger Brut, Bollinger Brut, Billecart-Salmon Brut, Pol Roger Brut, Dom Pérignon Brut, G.H.Mumm, Perrier-Jouët Grand Brut, Piper-Heidsieck Brut. In a cocktail, I prefer the fizz of a Franciacorta or Crémant over Champagne itself (note to self: snobbery considered harmful).
...
- *#ingredient([White Crème de Menthe])⇌ #ingredient([Peppermint Schnapps])*: #ingredient([Giffard Menthe Pastille]), #ingredient([Leroux]), #ingredient([De Kuyper]).
...
- *#ingredient([Gin]) ⇌ #ingredient([Jonge Jenever])*, slightly more herbal vs. slightly fuller bodied. \
  Other options: #ingredient([Akvavit]), #ingredient([Cachaça]), #ingredient([Pinga]), #ingredient([Obstler]), #ingredient([Quetsch Schnapps]), #ingredient([Slivovitz]), #ingredient([Eau de Vie]), #ingredient([Vodka]), #ingredient([White Rum]), #ingredient([Tequila]). #ingredient([Whiskey]) tends to get boozy. \
	*#ingredient([Gin])s*: #ingredient([Beefeater]), #ingredient([Bombay Sapphire]), #ingredient([Hendrick’s]), #ingredient([Tanqueray No. 10]).
...
- *#ingredient([Dry Vermouth]) ⇌ #ingredient([Dry White Port]) ⇌ #ingredient([Sherry Fino])*. Ballpark stuff. \
	*#ingredient([Dry Vermouth])s*: #ingredient([Noilly Prat (Extra) Dry]), #ingredient([Dolin Dry]), #ingredient([Mancini Secco]), #ingredient([Yzaguirre]), #ingredient([Lo Fi]), #ingredient([Tximista Blanco]).
...
- *#ingredient([Sparkling Wine])#indexIn([Bubbles]) ⇌ #ingredient([Champagne]) ⇌ #ingredient([Crémant de Quelqu’part]) ⇌ #ingredient([Cava]) ⇌ #ingredient([Franciacorta]) ⇌ #ingredient([Prosecco])*. \
